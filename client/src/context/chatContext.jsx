...
// con
const updateCurrentChatt = useCallback((chat) => {
  setCurrentChat(chat);
}, []);
...
const createChatss = useCallback(async (firstId, secondId) => {
  const response = await postrequest(
    `${baseUrl}/chats`,
    JSON.stringify({
      firstId,
...
const sendTextMessagee = useCallback(
  async (textMessage, sender, currentChatId, setTextMessage) => {
    console.log('sender', sender);
    if (!textMessage) return console.log('You must type something !');
...
const markAllNotificationsAsReadd = useCallback((notifications) => {
  const mNotifications = notifications.map((n) => {
    return { ...n, isRead: true };
  });
  setNotifications(mNotifications);
...
const markNotificationsAsReadd= useCallback(
  (n, userChats, user, notifications) => {
    //find chat to open
...
const markThisUserNotificationsAsReadd = useCallback(
  (thisUserNotifications, notifications) => {
    //mark notifications as read
